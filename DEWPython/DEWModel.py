...
# Equation imports
import numpy as np
from . import DEWEquations

# Pandas is used for the custom sheet reading
import pandas as pd

# sys, threading, subprocess, os, and platform all correspond to uses within the supcrt functions
import sys
import threading
import subprocess
import os
...
# directory holding the bundled SUPCRT executables and their support/data files
resourcesDir = op.join(op.dirname(op.abspath(__file__)), 'resources')

def _resolve_supcrt_executable(version='96'):
    '''Picks the bundled SUPCRT executable to run for the current operating system, makes sure
    it is marked executable, and returns its absolute path.
    ---Input---
    version        - '96' selects SUPCRT96, anything else selects SUPCRTBL
    ---Output---
    The absolute path of the executable to run. Raises RuntimeError if no bundled binary exists
    for the current OS/version combination, since DEWPython only ships a handful of prebuilt
    ...
    if version != '96':
        if system != 'Windows':
            raise RuntimeError(
                "SUPCRTBL is only bundled as a Windows executable (resources/SUPCRTBL.exe) and "
                f"cannot be run on {system}. Use run_supcrt(version='96') instead (also has a "
                "macOS build), or supply your own SUPCRTBL binary for this platform."
            )
        filename = 'SUPCRTBL.exe'
    else:
        if system == 'Windows':
            filename = 'supcrt96.exe'
        elif system == 'Darwin':
            filename = 'supcrt96.x'
        else:
            raise RuntimeError(
                f"SUPCRT96 has no bundled executable for {system}. Bundled binaries only cover "
                "Windows (supcrt96.exe) and macOS (supcrt96.x)."
            )
    exePath = op.join(resourcesDir, filename)
    if not op.isfile(exePath):
        raise FileNotFoundError(f"Expected SUPCRT executable not found: {exePath}")
    if system != 'Windows' and not os.access(exePath, os.X_OK):
        os.chmod(exePath, os.stat(exePath).st_mode | 0o111)
    return exePath
                ...
                if self.WaterFreeEq == 'Custom':
                    try:
                        if self.GibbsH2O[i] == 0:
                            ...
                            waterLst.append(self.GibbsH2O[i])
                    except IndexError:
                        waterLst.append(0)
                else:
...
######################################
    ...
    def run_supcrt(self, version = '96'):
        '''A function that runs the pre-compiled SUPCRT96/SUPCRTBL found in the resources folder.
        Picks the executable matching the current operating system; see _resolve_supcrt_executable.
        '''
        supPath = _resolve_supcrt_executable(version)
        self.proc = subprocess.Popen([supPath], stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=resourcesDir)
        ...
        while(self.proc.poll() is None):
            ...
            if '.txt' in var:
                # Bare filename: SUPCRT runs with resourcesDir as its cwd, so its output lands
                # there too. Consumers (calculate_supcrt / calculate_supcrt_special) resolve
                # this against resourcesDir themselves.
                self.supcrtFile = var
            inp=bytearray(var +'\n', sys.stdin.encoding)
    ...
    def calculate_supcrt_special(self, customFile = None):
        ...
        if customFile != None:
            filename = customFile if op.isabs(customFile) else op.join(os.getcwd(), customFile)
    ...
    def supcrt_inp(self, rxn_lst, title, reaction_type = 'psat'):
        ...
        for reaction in rxn_lst:
            supPath = _resolve_supcrt_executable('96')
            self.proc = subprocess.Popen([supPath], stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=resourcesDir)
